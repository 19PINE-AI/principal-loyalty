#!/usr/bin/env bash
# Extend multi-seed n=5 coverage to vendors that bracket the calibrated /
# intermediate / over-refuse transition zone:
#   claude-opus (calibrated, 17%)
#   glm-4.6     (intermediate, 49%)
#   gpt-5-mini  (over-refuse, 62%)
# Already done at n=5: claude-sonnet, deepseek, gemini-3-flash, gpt-5, qwen-27b
set -uo pipefail
...
# 3 vendors x 4 extra seeds (seed1 = the existing run)
for spec_label in "claude-opus:claude_opus" "glm-4.6:glm46" "gpt-5-mini:gpt5mini"; do
  spec="${spec_label%%:*}"
  label="${spec_label##*:}"
...
echo "[DONE] multi-seed expansion"
