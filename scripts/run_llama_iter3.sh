#!/usr/bin/env bash
# Llama per-token KL iter3: sample from llama_pertoken_kl_iter2_merged,
...
PREV_MERGED="runs/llama_pertoken_kl_iter2_merged"
TRAJ="data/pertoken_kl/llama_iter3_trajectories.jsonl"
TOPK="data/pertoken_kl/llama_iter3_topk.jsonl"
ADAPTER="runs/llama_pertoken_kl_iter3"
MERGED="runs/llama_pertoken_kl_iter3_merged"
EVAL_DIR="runs/phase5_pertoken_kl_llama_iter3"

# 1. Sample student under iter2 checkpoint
if [ ! -f "$TRAJ" ] || [ "$(wc -l < $TRAJ)" -lt 25 ]; then
  ...
  nohup /home/ubuntu/aoi-env/bin/python -m vllm.entrypoints.openai.api_server \
    ...
    --port $STUDENT_PORT > logs/vllm_llama_iter3_sample.log 2>&1 &
  pid=$!
  ...
  OUT_PREFIX="data/pertoken_kl/llama_iter3_sample"
  /home/ubuntu/aoi-env/bin/python scripts/onpolicy_distill_iter.py \
    ...
    --student-arm plain --teacher-arm scaffolded 2>&1 | tee logs/llama_iter3_sample.log | tail -10
  cp "${OUT_PREFIX}_trajectories.jsonl" "$TRAJ"
fi
echo "[llama-iter3] $(wc -l < $TRAJ) student trajectories"
...
if [ ! -f "$TOPK" ] || [ "$(wc -l < $TOPK)" -lt 30 ]; then
  ...
  nohup /home/ubuntu/aoi-env/bin/python -m vllm.entrypoints.openai.api_server \
    ...
    --enforce-eager \
    --port $TEACHER_PORT > logs/vllm_llama_iter3_teacher.log 2>&1 &
  pid=$!
  ...
  TOKENIZER_NAME="runs/llama_pertoken_kl_iter2_merged" \
  /home/ubuntu/aoi-env/bin/python scripts/pertoken_kl_collect.py \
    ...
    | tee logs/llama_iter3_collect.log | tail -10
fi
echo "[llama-iter3] $(wc -l < $TOPK) topk records"
...
if [ ! -d "$ADAPTER" ] || [ -z "$(ls $ADAPTER 2>/dev/null)" ]; then
  ...
  PL_MODEL_ID="$PREV_MERGED" PL_KL_PATH="$TOPK" PL_OUT_DIR="$ADAPTER" \
    ...
    | tee logs/llama_iter3_train.log | tail -20
fi
...
if [ ! -f "$EVAL_DIR/scored.jsonl" ] || [ "$(wc -l < $EVAL_DIR/scored.jsonl)" -lt 100 ]; then
  ...
  nohup /home/ubuntu/aoi-env/bin/python -m vllm.entrypoints.openai.api_server \
    ...
    --dtype bfloat16 --max-model-len 8192 --gpu-memory-utilization 0.20 \
    --port $STUDENT_PORT > logs/vllm_llama_iter3_eval.log 2>&1 &
  pid=$!
  t=0; until curl -sf http://localhost:$STUDENT_PORT/v1/models >/dev/null 2>&1; do sleep 10; t=$((t+10)); if ! kill -0 $pid 2>/dev/null; then exit 1; fi; if [ $t -ge 600 ]; then exit 1; fi; done
...
for label, rs in [('full 50-item', rows), ('original 36-item', rows36)]:
    ...
    print(f'[summary] llama_pertoken_kl_iter3 ({label}): n={n} harm={harm} ({100*harm/n:.0f}%) leak={leak} bound={bound} MI={mi}')
"
kill_vllm
echo "[DONE] llama-iter3"
