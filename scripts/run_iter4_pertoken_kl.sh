#!/usr/bin/env bash
# Per-token KL iter4: sample student from iter3 checkpoint, collect Qwen3-32B
# teacher logprobs, train, eval. Confirms K=3 plateau by showing iter4 also
...
if [ ! -f "$TRAJ" ] || [ "$(wc -l < $TRAJ)" -lt 25 ]; then
  ...
  nohup /home/ubuntu/aoi-env/bin/python -m vllm.entrypoints.openai.api_server \
    ...
    --port $STUDENT_PORT > logs/vllm_iter4_sample.log 2>&1 &
  pid=$!
  ...
  OUT_PREFIX="data/pertoken_kl/iter4_sample"
  /home/ubuntu/aoi-env/bin/python scripts/onpolicy_distill_iter.py \
    ...
    --student-arm plain --teacher-arm scaffolded 2>&1 | tee logs/pertoken_kl_iter4_sample.log | tail -10
  cp "${OUT_PREFIX}_trajectories.jsonl" "$TRAJ"
fi
echo "[iter4] $(wc -l < $TRAJ) student trajectories"
...
if [ ! -f "$TOPK" ] || [ "$(wc -l < $TOPK)" -lt 90 ]; then
  ...
  nohup /home/ubuntu/aoi-env/bin/python -m vllm.entrypoints.openai.api_server \
    ...
    --enforce-eager \
    --port $TEACHER_PORT > logs/vllm_iter4_teacher.log 2>&1 &
  pid=$!
...
echo "[iter4] $(wc -l < $TOPK) topk records"
