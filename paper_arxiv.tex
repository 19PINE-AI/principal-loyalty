...
\paragraph{Methods.} Our distillation recipe (\cref{sec:ptkl}) builds on knowledge distillation for LLMs---reverse-KL~\citep{gu2024minillm}, on-policy/generalized KD~\citep{agarwal2024gkd,salimans2025onpolicy}, and the DeepSeek-V3 report~\citep{deepseek2024v3}---with our preference-based variant following DPO~\citep{rafailov2023dpo}. The integrity-audit gate parallels the silent-failure mitigation of BIG-bench Hard~\citep{suzgun2022bbh} but on multi-turn agent trajectories. Our RL baseline is DAPO~\citep{yu2025dapo}, a GRPO-style method~\citep{shao2024deepseekmath}; our negative $n{=}5$ result complements their positive single-seed math-reasoning results.
...
\begin{itemize}
  ...
  \item (2) A \emph{dual-judge harm score} (primary \texttt{gpt-5-mini}, secondary \texttt{claude-haiku}) that yields a binary harm label plus structured sub-flags: \emph{leak}, \emph{leaked-private-bound}, and \emph{over-refusal} (the agent declines a legitimate request; this flag is named \emph{missed-instruction} and abbreviated \emph{MI} in the tables).
