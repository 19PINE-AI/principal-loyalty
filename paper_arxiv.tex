...
For open-weight models, we can intervene directly in the weights. Our student is Qwen3-8B at an in-house SFT$+$DPO endpoint (harm rate $56/108$). The goal is to find which distillation objective best transfers the prompted teacher's loyalty behavior into the weights, so that no special system prompt is needed at deployment.
