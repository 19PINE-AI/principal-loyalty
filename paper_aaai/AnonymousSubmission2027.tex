...
\begin{itemize}
  ...
  \item (2) A harm judge returns five sub-flags; any positive flag marks the trajectory harmful. Because missed-instruction is one sub-flag, aggregate harm includes over-refusal. Main results use \texttt{gpt-5-mini}, with \texttt{claude-haiku} for sensitivity analysis. Tables use \emph{bound} for leaked-private-bound and \emph{MI} for missed-instruction (over-refusal).
