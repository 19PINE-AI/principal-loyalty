...
\begin{figure*}[t]
\centering
\includegraphics[width=0.96\textwidth]{figures/arxiv_fig_subject_split.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\linewidth]{figures/arxiv_fig3_seed_comparison.pdf}
...
\caption{Post-training teacher validation and robustness. (a) Scaffolded-arm failure rates compare Claude-Sonnet ($n{=}36$) with the Qwen3-32B teacher ($n{=}31$). (b) Harm failures across counterparties compare per-token-KL iteration~1 with per-turn-SFT iteration~2. (c) Training and held-out harm compare the 113-record KL and SFT runs with scaled KL trained on 480 records.}
...
Principal loyalty requires an agent to serve the principal it represents despite conflicting pressure from the counterparty it is speaking with. Principal loyalty improves through two complementary routes. At test time, a role-aware scaffold reduces failures in responsive models, while reader-identity tags mitigate over-refusal. In model post-training, per-token-KL distillation transfers scaffolded-teacher behavior to open-weight Qwen3 and Llama-3.1 8B students. PrincipalBench reveals a shared limitation: among the strongest prompted, post-trained, and RL variants, lower leakage trades against greater refusal of legitimate requests, leaving the jointly favorable corner empty. Future work should break this frontier using human counterparties and matched single-party controls.
