...
\begin{enumerate}
\item \textbf{Problem and measurement:} six loyalty-failure modes and the 75-item, multi-turn PrincipalBench, which distinguishes selective behavior from blanket over-refusal across 13 models (\cref{sec:problem,sec:bench,sec:split}).
\item \textbf{Prompting:} a seven-rule scaffold holding Claude-Sonnet at $19.4\%$ harm and all nine selective models at $\le20\%$, but not rescuing the over-refusing cluster (\cref{sec:prompt}).
\item \textbf{Distillation:} per-token KL transfers prompted-teacher behavior across Qwen3 and Llama-3.1 8B model families (\cref{sec:ptkl}).
\item \textbf{Trade-off:} prompting, distillation, and scalar-reward RL all remain on a common leak/over-refusal frontier rather than reaching the jointly favorable region (\cref{sec:manifold}).
\end{enumerate}
...
\label{sec:kiter}
Each iteration re-samples trajectories from the previous checkpoint and re-distills against the same prompted teacher. Repetition traces a path \emph{along} the trade-off rather than converging to an optimum. \Cref{tab:kiter} focuses on Qwen3-8B, the primary student; the Llama transfer is summarized below.
...
\bottomrule
\end{tabular}
\caption{\textbf{$K$-iteration distillation for Qwen3-8B} (single-seed failures per 108; MI = missed-instruction). Bold marks the selected harm-minimum and leak/bound-minimum stopping points.}
\label{tab:kiter}
\end{table}
