...
\bottomrule
\end{tabular}
\caption{Six loyalty-failure categories. The first five measure failures toward the principal; \emph{Sanity} penalizes blanket over-refusal on a cooperative request.}
\label{tab:cells}
\end{table*}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/arxiv_fig_subject_split.pdf}
\caption{The selective/over-refusing split persists on held-out items. (a) Five-seed means ($\pm1\sigma$) on the 36-item core reveal a 34-point gap. (b) Held-out rates use each subject's valid cells; labels give harmful/valid cells.}
\label{fig:xsubj}
\end{figure*}
...
\bottomrule
\end{tabular}
\caption{Cluster harm composition. Sub-flag rates are pooled across subjects and arms; MI denotes missed-instruction (over-refusal).}
\label{tab:profile}
\end{table}
...
\section{Improving principal loyalty at test time}
\label{sec:prompt}
...
\bottomrule
\end{tabular}
\caption{Inference-time scaffolding is model-dependent. Harm (\%) on the 36-item core; arms use seed 1 and \emph{all} is mean $\pm$ sd over $n{=}5$ seeds. Bold marks the over-refusing cluster.}
...
\caption{Multi-seed model post-training comparison. Iteration~1 minimizes mean harm and missed-instruction failures; iteration~2 minimizes leakage and private-bound failures. Error bars show $\pm1\sigma$ for the KL checkpoints (iteration~1: $n{=}5$; iteration~2: $n{=}4$); the base uses five seeds.}
...
\caption{Post-training validation and robustness. (a) Scaffolded-arm failures for Claude-Sonnet ($n{=}36$) and the Qwen3-32B teacher ($n{=}31$). (b) Harm across counterparties for per-token-KL iteration~1 and per-turn-SFT iteration~2. (c) Training and held-out harm for the 113-record runs and 480-record scaled-KL run.}
...
\bottomrule
\end{tabular}
\caption{Iterative model post-training for Qwen3-8B (per-token-KL distillation; single-seed failures per 108; MI = missed-instruction). Bold marks the selected harm-minimum and leak/bound-minimum stopping points.}
\label{tab:kiter}
\end{table}
...
None of the five controls in \cref{tab:floor} reaches the empty corner, evidence for a structural limit. RL from the best post-trained checkpoint moves backward, an apparent single-seed path around the frontier disappears under multi-seed replication, and naive data scaling also worsens performance.

\paragraph{Compute.} DAPO training used one NVIDIA RTX Pro 6000 Blackwell; rank-32 LoRA peaked at approximately 64~GB of GPU memory, whereas a full-parameter actor exceeded 94~GB and did not fit.
...
\bottomrule
\end{tabular}
\caption{Five controls fail to break the frontier.}
\label{tab:floor}
\end{table}

\paragraph{Evidence from an industrial-scale system.} The Claude Opus 4.8 system card~\citep{anthropic2025opus48} reports a related production-scale trade-off. A business-skills and adversarial-robustness regimen used for Opus 4.7 contributed to dishonesty; removing it for Opus 4.8 recovered honesty but weakened scam resistance and negotiation. Although its axes differ from ours (truthfulness vs.\ over-refusal, robustness vs.\ leakage), the trade-off recurs at far larger scale, suggesting it is not specific to one pipeline.
...
\bottomrule
\end{tabular}
\caption{Judge sensitivity of the post-training improvement (per-token-KL; mean failures per 108, $n{=}5$). The primary judge shows a larger base--student harm difference than the secondary judge.}
\label{tab:judge}
\end{table}

\paragraph{Generalization across counterparties and data splits.} Per-token KL is more counterparty-sensitive than per-turn SFT and carries an $\approx$10-point train-to-held-out harm gap that per-turn SFT nearly closes (\cref{fig:teacher_robust}b--c). Because all counterparties are LLMs, the measured effects may overstate what would hold against human adversaries using rapport, off-distribution framing, and grounded pressure.

\paragraph{Construct validity and sample size.} Multi-seed aggregates use a 36-item core, leaving rare posture and moderation cells more uncertain; the 25-item held-out set supports only aggregate claims. Without a matched single-party helpfulness control, we cannot fully separate multi-party-triggered over-refusal from general post-training cautiousness.

\section{Conclusion}

Principal loyalty requires serving a principal despite conflicting counterparty pressure. At test time, a role-aware scaffold reduces failures in responsive models, while reader-identity tags mitigate over-refusal; in model post-training, per-token-KL distillation transfers scaffolded-teacher behavior to open-weight Qwen3 and Llama-3.1 8B students. Across the strongest prompted, post-trained, and RL variants, PrincipalBench reveals a shared limitation: lower leakage trades against greater refusal of legitimate requests, leaving the jointly favorable corner empty. Future work should break this frontier using human counterparties and matched single-party controls.
