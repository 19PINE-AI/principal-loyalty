...
\caption{\textbf{Headline result: a leak/over-refusal Pareto frontier (preview, expanded in \cref{sec:manifold}).} Each marker is one model variant, scored over the 36 core items under three prompt conditions (\emph{arms}). The $x$-axis is leak rate, $y$ is over-refusal rate (missed-instruction), and the label is the harm count out of 108 item-arm cells. In variant labels, \emph{iter1}, \emph{iter2}, and \emph{iter3} denote successive distillation iterations. The dashed line is the Pareto frontier: no point below and to its left is reachable, so the jointly favorable lower-left corner (shaded) is empty. The prompted Claude teacher and the per-token-KL 8B student sit on the \emph{same} frontier at different operating points, with neither dominating the other. A DAPO RL baseline also fails to reach the empty corner. Both mechanisms we propose move \emph{along} this frontier rather than crossing it.}
\label{fig:manifold}
\end{figure}
...
\paragraph{Prompt-scaffold performance by model cluster.} Across the nine selective subjects the scaffold holds the $n{=}5$ aggregate harm to $\le 20\%$ (\cref{tab:prompt_grid}), and it \emph{actively helps} the higher-starting members. This effect is most clearly shown in Qwen3-32B, where the scaffold reduces harm from $25\%$ on the plain arm to $12\%$. On Claude-Sonnet it achieves $21/108$ ($19.4\%$) harm across the three arms with a single seed. The over-refusing cluster shows no meaningful improvement. The full per-arm results appear in \Cref{tab:prompt_grid}.
...
\subsection{Headline and multi-seed results}
We collect one round of on-policy data (113 teacher turn-records, 28{,}486 token-level signals) and train 3 epochs of QLoRA at rank 16. On Qwen3-8B, per-token KL is the only single-iteration recipe that improves \emph{all four} scored axes at once against the SFT$+$DPO base: harm, leak, leaked-private-bound, and over-refusal (missed-instruction). Iteration-1 counts are in \cref{tab:kiter}, and its operating point in \cref{fig:manifold}. Its aggregate leak rate even drops \emph{below} the prompted Claude teacher's. Across five matched evaluation seeds, mean harm falls from $47.8$ failures per 108 cells for the base to $39.2$ for per-token-KL iteration~1; the corresponding leak, bound, and over-refusal means move from $15.8$, $4.6$, and $44.4$ to $13.8$, $2.8$, and $37.2$ (\cref{fig:seed_comparison}). Two caveats temper these observations. First, the harm difference is much smaller under the secondary re-judge (\cref{sec:limits}). Second, the single-seed point in \cref{fig:manifold} sits at the harm-favorable end of the seed distribution, so the multi-seed mean (\cref{fig:seed_comparison}) runs a few cells higher.

The five-seed DAPO-style RL replication does not reproduce its single-seed improvement (\cref{sec:manifold}).

\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{figures/arxiv_fig3_seed_comparison.pdf}
\caption{\textbf{Multi-seed comparison with the SFT$+$DPO base.} Per-token-KL iteration~1 has the lowest mean harm ($47.8\to39.2$) and missed-instruction count ($44.4\to37.2$), whereas iteration~2 has the lowest mean leak ($15.8\to11.2$) and private-bound count ($4.6\to2.5$). Bars show mean failures per 108 cells across evaluation seeds; error bars show $\pm1\sigma$ for the per-token-KL checkpoints. The base and iteration~1 use five seeds; iteration~2 uses four. At iteration~1, cells firing under all five seeds fall $15\to6$ on harm and $15\to5$ on over-refusal.}
\label{fig:seed_comparison}
\end{figure}

\subsection{The $K$-iteration trajectory}
\label{sec:kiter}
The recipe can be repeated: re-sample student trajectories from each iteration-$K$ checkpoint, then re-distill. This traces a path \emph{along} the trade-off rather than converging to an optimum. \Cref{tab:kiter} focuses on Qwen3-8B, the primary student; \cref{fig:kiter} shows how the two model families differ in shape.
...
\bottomrule
\end{tabular}
\caption{\textbf{$K$-iteration distillation trajectory for Qwen3-8B} (failures per 108 cells at single seed; MI $=$ missed-instruction, the over-refusal flag; bound $=$ leaked private bound). Each iteration re-samples student trajectories from the previous checkpoint and re-distills against the same prompted teacher. Bold marks the selected harm-minimum and leak/bound-minimum stopping points.}
\label{tab:kiter}
\end{table}

\textbf{Qwen moves back and forth along the leak/harm trade-off.} Iteration~1 is the harm-minimum, iteration~2 the leak/bound-minimum, iterations 3--4 regress, and iteration~5 returns to iteration~1's harm level but at the series' \emph{worst} leak/bound: it traces the trade-off rather than crossing it. In the multi-seed comparison, both stopping points have lower mean harm than the base (\cref{fig:seed_comparison}). \textbf{Llama descends monotonically} to iteration~3 and plateaus. The shape is base-model-dependent, but in both families each iteration trades one axis for another and never strictly dominates the previous iteration.
...
\label{sec:selfval}
\textbf{Cross-family transfer.} We repeat the recipe on a second family: Llama-3.1-8B distilled from a Llama-3.1-70B-Instruct-AWQ teacher (same family, shared tokenizer). It descends comparably to the Qwen3 result (\cref{fig:kiter}) and carries a comparable train-to-held-out gap (harm $15.7\%\to26.7\%$). The recipe is therefore not specific to a single base family.
...
\paragraph{Judge sensitivity for 8B students.} The two judges agree perfectly on frontier trajectories whose harm decision is unambiguous ($\kappa=1.0$ on a balanced 60-item three-judge subset) but only agree weakly on the borderline per-token-KL 8B student ($\kappa=0.08$, the secondary judge being systematically more lenient), and agree moderately on the base ($\kappa=0.42$). This materially affects the observed harm difference. Re-judging all $1{,}080$ base and student trajectories with the secondary judge preserves the \emph{direction} of the per-token-KL harm reduction but makes the difference much smaller, and the judge-independent leak probe also shows only a small difference (\cref{tab:judge}). The caveat is specific to the borderline 8B regime. The frontier-model split of \cref{sec:split} is scored on trajectories with an unambiguous harm decision ($\kappa=1.0$), so the paper's central structural claim does not rest on the contested judge.
...
\bottomrule
\end{tabular}
\caption{\textbf{Judge sensitivity of the per-token-KL comparison.} Mean failing cells per 108 over $n{=}5$ seeds, per-token-KL student vs.\ SFT$+$DPO base. The primary judge shows a larger base--student harm difference than the secondary judge; the judge-independent leak probe shows a small difference.}
\label{tab:judge}
\end{table}
