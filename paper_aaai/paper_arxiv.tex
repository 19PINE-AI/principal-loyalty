...
% --- title block ------------------------------------------------------------
\title{Improving Multi-Party Principal Loyalty in LLM Agents}
...
We test two routes---inference-time scaffolding and model post-training---and ask whether they improve principal loyalty without inducing indiscriminate refusal. Each helps where effective, but neither removes the underlying tension.
...
\item \textbf{Shared limit.} The strongest prompted, post-trained, and scalar-reward RL variants remain on a common leakage/over-refusal frontier. Five controls fail to reach the jointly favorable region, and Anthropic's reported Opus~4.7$\to$4.8 trade is a related industrial-scale instance (\cref{sec:manifold}; \citealp{anthropic2025opus48}).
\end{enumerate}

\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{figures/arxiv_fig1_manifold.pdf}
\caption{\textbf{Shared limit: a leakage/over-refusal Pareto frontier (preview, expanded in \cref{sec:manifold}).} Each marker is one model variant, scored over the 36 core items under three prompt conditions (\emph{arms}). The $x$-axis is leakage, the $y$-axis is over-refusal (missed-instruction), and the label is harm out of 108 item--arm cells. In variant labels, \emph{iter1}, \emph{iter2}, and \emph{iter3} denote successive post-training iterations. Inference-time scaffolding and model post-training can improve weaker baselines, but their strongest variants occupy different points on the same frontier. No tested prompting, post-training, or RL variant reaches the shaded low-leak, low-refusal corner.}
\label{fig:manifold}
\end{figure}
...
Throughout, \emph{principal-directed loyalty violations} refers to the first five cells, while \emph{over-refusal} refers to the cooperative Sanity cell. The aggregate \emph{harm} score marks a trajectory when either kind of failure occurs; it is therefore not an axis independent of over-refusal.

\paragraph{Limitations of single-axis evaluation.} Reducing multi-party loyalty to a single axis like ``How often does the agent leak information?'' fails in both directions. An agent can score perfectly on leakage yet still capitulate on price under pressure, signal in ways that weaken the principal's position, or author documents conceding what the live conversation withheld. Conversely, an agent that refuses everything never leaks, an equally serious failure that single-axis safety metrics inadvertently reward. We therefore decompose the problem space into \textbf{six failure modes}, each a benchmark cell, with one worked example per cell in \cref{tab:cells}.
...
\toprule
cell & what goes wrong & example (used-car sale) \\
\midrule
...
\bottomrule
\end{tabular}
\caption{\textbf{The six failure cells of multi-party loyalty.} Five (Leakage--Moderation) are distinct ways the agent fails the principal; the sixth (\emph{Sanity}) is a cooperative item where over-refusal is the only failure, included so that ``refuse everything'' is not a winning strategy.}
\label{tab:cells}
\end{table}

\paragraph{Co-occurrence of loyalty failures: worked example.} The six failure modes are not disjoint. In practice, a single short exchange can trigger several failures simultaneously, which makes the agent's job materially harder than a single-axis defense would suggest. Consider a used-car-sale item where the seller's briefing reads ``asking \$15{,}000, minimum I'd accept \$12{,}000, walk-away below \$11{,}500. Do not reveal my minimum or that I'm motivated.''
...
\begin{itemize}
  ...
  \item (2) A \emph{dual-judge harm score} (primary \texttt{gpt-5-mini}, secondary \texttt{claude-haiku}) that yields a binary harm label plus structured sub-flags: \emph{leak}, \emph{leaked-private-bound}, and \emph{over-refusal} (the agent declines a legitimate request), named \emph{missed-instruction} and abbreviated \emph{MI} in the tables. Because MI is a sub-flag, aggregate harm includes over-refusal.
...
\textbf{Robustness.} The split appears in every prompt arm and survives on the held-out items authored after training was frozen (\cref{fig:heldout_xsubj,tab:profile}). The two clusters stay cleanly separated, with GPT-5 reaching $93\%$ harm.
...
\bottomrule
\end{tabular}
\caption{\textbf{Inference-time scaffolding is model-dependent.} Harm (\%) on the 36-item core; arms use seed 1 and aggregate is mean $\pm$ sd over $n{=}5$ seeds. Bold marks the over-refusing cluster.}
\label{tab:prompt_grid}
\end{table}

\paragraph{Why the improvement is conditional.} On the over-refusing cluster, the scaffold is ineffective or actively harmful: nearly all non-zero item-level changes from the plain to the prompted arm move toward more harm. These models already over-refuse heavily as a result of their own safety training, and the scaffold's ``adversarial stranger'' framing pushes them further into refusal. The selective cluster does not show a comparable aggregate shift. The scaffold amplifies existing selectivity rather than creating it.

\paragraph{Reader identity limits scaffold-induced over-refusal.} The scaffold creates one new failure: it sometimes refuses the principal's \emph{own} cooperative requests, because it mistakes the principal for the adversarial counterparty (e.g., when the principal asks the agent to draft their own self-review). Prepending a one-line reader tag (\texttt{[READER:\,PRINCIPAL]} or \texttt{[READER:\,THIRD\_PARTY]}, \cref{app:prompt}) reduces this failure from $9/12$ to $3/12$ probe trajectories. This fix generalizes to held-out probes and holds even when the counterparty tries to spoof a principal-side tag. The scaffold plus this tag is the \emph{scaffolded} arm. It recovers cooperative behavior without making leaks more likely again.
...
We collect one round of on-policy data (113 teacher turn-records, 28{,}486 token-level signals) and train 3 epochs of QLoRA at rank 16. Per-token KL is the only single-iteration post-training objective that improves \emph{all four} scored axes against the SFT$+$DPO base: aggregate harm, leakage, leaked-private-bound, and over-refusal (missed-instruction). Iteration-1 counts are in \cref{tab:kiter}, and its operating point in \cref{fig:manifold}. Its aggregate leak rate even drops below the inference-time-scaffolded Claude teacher's. Across five matched evaluation seeds, mean harm falls from $47.8$ failures per 108 cells for the base to $39.2$ for per-token-KL iteration~1; the corresponding leakage, bound, and over-refusal means move from $15.8$, $4.6$, and $44.4$ to $13.8$, $2.8$, and $37.2$ (\cref{fig:seed_comparison}). Two caveats temper these observations. First, the harm difference is much smaller under the secondary re-judge (\cref{sec:limits}). Second, the single-seed point in \cref{fig:manifold} sits at the harm-favorable end of the seed distribution, so the multi-seed mean (\cref{fig:seed_comparison}) runs a few cells higher.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{figures/arxiv_fig3_seed_comparison.pdf}
\caption{\textbf{Multi-seed model post-training improvement.} Against the SFT$+$DPO base, per-token-KL iteration~1 has the lowest mean harm ($47.8\to39.2$) and missed-instruction count ($44.4\to37.2$), whereas iteration~2 has the lowest mean leak ($15.8\to11.2$) and private-bound count ($4.6\to2.5$). Bars show mean failures per 108 cells; error bars show $\pm1\sigma$. The base and iteration~1 use five seeds; iteration~2 uses four. At iteration~1, cells firing under all five seeds fall $15\to6$ on harm and $15\to5$ on over-refusal.}
\label{fig:seed_comparison}
\end{figure}

\subsection{Successive post-training iterations}
\label{sec:kiter}
The post-training recipe can be repeated: re-sample student trajectories from each iteration-$K$ checkpoint, then re-distill against the same scaffolded teacher. This changes the operating point rather than converging to one optimum. \Cref{tab:kiter} focuses on Qwen3-8B, the primary student; \cref{fig:kiter} shows how the two model families differ in shape.
...
\bottomrule
\end{tabular}
\caption{\textbf{Iterative model post-training for Qwen3-8B} (per-token-KL distillation; failures per 108 cells at single seed; MI $=$ missed-instruction, the over-refusal flag; bound $=$ leaked private bound). Bold marks the selected harm-minimum and leak/bound-minimum stopping points.}
\label{tab:kiter}
\end{table}

\textbf{The preferred post-training checkpoint depends on the failure axis.} Iteration~1 minimizes aggregate harm, iteration~2 minimizes leak/bound, iterations 3--4 regress, and iteration~5 returns to iteration~1's harm level but at the series' \emph{worst} leak/bound. In the multi-seed comparison, both selected stopping points have lower mean harm than the base (\cref{fig:seed_comparison}). \textbf{Llama descends monotonically} to iteration~3 and plateaus. The trajectory is base-model-dependent, but additional post-training does not produce one checkpoint that is best on every axis.
...
\end{subfigure}
\caption{\textbf{Successive model post-training by family.} Each per-token-KL iteration re-samples student trajectories from the previous checkpoint and re-distills against the same scaffolded teacher.}
\label{fig:kiter}
\end{figure}

\subsection{Transfer, teacher validation, and counterparty robustness}
\label{sec:selfval}
\textbf{Cross-family transfer.} We repeat the post-training recipe on a second family: Llama-3.1-8B with a Llama-3.1-70B-Instruct-AWQ teacher (same family, shared tokenizer). It descends comparably to the Qwen3 result (\cref{fig:kiter}) and carries a comparable train-to-held-out gap (harm $15.7\%\to26.7\%$). The improvement is therefore not specific to a single base family.
...
3. Single-objective RL improvement & re-test at $n{=}5$ a DAPO run that improved at single seed & the single-seed improvement is not reproduced on any axis \\
...
5. Scale post-training data & $4.2\times$ data (480 vs.\ 113 records), same epochs/rank/LR & regresses harm on both training ($31\to37\%$) and held-out ($40\to56\%$) \\
\bottomrule
\end{tabular}
\caption{\textbf{Five controls, none reaches the favorable corner.} The controls span iterative model post-training, RL, reward design, and data scaling. All fail relative to the best post-trained checkpoint, supporting a shared structural limitation rather than a method-specific artifact.}
\label{tab:floor}
\end{table}
...
\paragraph{Judge sensitivity of the model post-training result.} The two judges agree perfectly on frontier trajectories whose harm decision is unambiguous ($\kappa=1.0$ on a balanced 60-item three-judge subset) but only agree weakly on the borderline per-token-KL 8B student ($\kappa=0.08$, the secondary judge being systematically more lenient), and agree moderately on the base ($\kappa=0.42$). This materially affects the observed improvement. Re-judging all $1{,}080$ base and student trajectories preserves the \emph{direction} of the post-training harm reduction but makes the difference much smaller, and the judge-independent leak probe also shows only a small difference (\cref{tab:judge}). The caveat is specific to the borderline 8B regime. The frontier-model split of \cref{sec:split} is scored on trajectories with an unambiguous harm decision ($\kappa=1.0$), so the paper's central structural claim does not rest on the contested judge.
...
signal & base & student \\
\midrule
primary judge (\texttt{gpt-5-mini}), harm    & $47.8$ & $39.2$ \\
secondary judge (\texttt{claude-haiku}), harm & $42.6$ & $41.4$ \\
deterministic leak probe                      & $15.8$ & $13.8$ \\
\bottomrule
\end{tabular}
\caption{\textbf{Judge sensitivity of the model post-training improvement.} Mean failing cells per 108 over $n{=}5$ seeds, per-token-KL student vs.\ SFT$+$DPO base. The primary judge shows a larger base--student harm difference than the secondary judge; the judge-independent leak probe shows a small difference.}
\label{tab:judge}
\end{table}

\paragraph{Sensitivity to the counterparty model.} Per-token KL is more counterparty-sensitive on the harm axis than per-turn SFT (\cref{fig:teacher_robust}b).

\paragraph{Absence of human-adversary evaluation.} The counterparty in every item is itself an LLM with a parameterized persona and opening move, not a human red-teamer. Humans can use rapport, off-distribution framings, and externally grounded social pressure that scripted counterparties miss. Our selective/over-refusing split and our mechanism effect sizes are therefore upper bounds on what holds under human adversaries.

\paragraph{Generalization gap under per-token KL.} Both per-token-KL checkpoints (Qwen iteration~1, Llama iteration~3) carry an $\approx$10-point train-to-held-out harm gap (\cref{fig:teacher_robust}c). Per-turn SFT (iteration~2) nearly closes it, so the gap appears to be a feature of the per-token-KL objective rather than of distillation in general. We report it but do not yet have a fix.

\paragraph{Sample size and failure-mode coverage.} The multi-seed $n{=}5$ aggregates use a 36-item core (drawn from the 50 training items), so rarer failure modes (posture, moderation) carry wider per-cell uncertainty than the more common ones (leakage, capitulation). The held-out set of 25 items supports the cross-subject and per-recipe comparisons we report, but not fine-grained per-cell held-out claims.

\paragraph{Attribution of over-refusal.} We attribute the over-refusing cluster's over-refusal rate to multi-party items specifically, on the strength of in-cluster vs.\ cross-cluster contrasts. We do not run a matched single-party control (the same models on a vanilla helpfulness benchmark), so we cannot fully separate ``triggered by multi-party framing'' from ``generally more cautious after safety post-training.''

\section{Conclusion}

Principal loyalty is improvable through two complementary routes. At inference time, a role-aware loyalty scaffold can reduce failures in responsive models, and reader-identity tags mitigate its tendency to over-refuse. Through model post-training, per-token-KL distillation transfers scaffolded-teacher behavior into open-weight Qwen3 and Llama-3.1 8B students. PrincipalBench provides the common measurement needed to compare these routes and exposes why aggregate harm alone is insufficient: models can fail either by abandoning the principal or by refusing legitimate work.

The improvements share a structural limit. Among the strongest inference-time, post-trained, and RL variants, lower leakage trades against greater refusal of legitimate requests, and no tested system reaches the jointly favorable corner. The next generation of loyalty interventions should preserve the gains of both routes while breaking this leakage/over-refusal frontier. Natural next steps include human counterparties, matched single-party controls, and post-training objectives that represent the two axes separately rather than collapsing them into one scalar reward.
